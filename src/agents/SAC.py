...
class SAC():
    ...
    def select_action_monte_carlo(self, state, M=10, deterministic=False):
        """
        Perform Monte Carlo sampling to estimate the action.
        :param state: The input state for action selection
        :param M: The number of Monte Carlo samples
        :param deterministic: Whether to use the mean action or sampled action
        :return: The estimated action
        """
        state_tensor = torch.from_numpy(state.astype(np.float32)).unsqueeze(0)
        state_tensor = CUDA(state_tensor) if next(self.policy.parameters()).is_cuda else state_tensor

        # Perform M Monte Carlo samples
        actions = [self.policy.select_action(state_tensor, deterministic) for _ in range(M)]
        
        # Convert list of tensors to tensor
        actions_tensor = torch.stack(actions)
        
        # Calculate the mean of the action distributions
        mean_action = torch.mean(actions_tensor, dim=0)
        
        # Return the mean action
        return CPU(mean_action).detach().numpy()
