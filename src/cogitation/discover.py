...
import copy
from .causal_graph_trainer.notears.nonlinear import NotearsMLP, notears_nonlinear
...
class Discover(object):
    ...
    def create_feature_matrix(self, S_t, A_t, S_t1):
        """
        Create the feature matrix from factorized state and action.

        Args:
        - S_t (np.ndarray): The numpy array of current states.
        - A_t (np.ndarray): The numpy array of actions.
        - S_t1 (np.ndarray): The numpy array of next states.

        Returns:
        - np.ndarray: The concatenated feature matrix.
        """
        return np.concatenate([S_t, A_t, S_t1], axis=1)
    
    
    def Reduce(self,W, epsilon):
        """
        Reduces the learned weighted adjacency matrix W to a smaller matrix
        that represents the causal graph G with dimensions [d_s x (d_s+1)].
        The reduction involves pruning edges based on the threshold epsilon.

        Args:
        - W (torch.Tensor): The learned weighted adjacency matrix of shape [2*d_s+1 x 2*d_s+1].
        - epsilon (float): Threshold parameter for determining the existence of an edge.

        Returns:
        - torch.Tensor: The reduced adjacency matrix of shape [d_s x (d_s+1)].
        """
        # W is assumed to be a square matrix with shape [2*d_s+1, 2*d_s+1]
        d_s = (W.shape[0] - 1) // 2
        
        # Extract the submatrix that corresponds to edges between {s_t, a_t} and {s_t+1}
        # This involves selecting the rows corresponding to s_t+1 and the columns corresponding to {s_t, a_t}
        W_reduced = W[d_s+1:, :d_s+1]
        
        # Apply thresholding to determine the existence of an edge
        W_reduced = torch.where(torch.abs(W_reduced) > epsilon, W_reduced, torch.zeros_like(W_reduced))
        
        return W_reduced
    
    def train_causal_graph(self, S_t, A_t, S_t1, d_s, lambda1, lambda2, epsilon):
        """
        Train the causal graph using NotearsMLP and perform reduction.

        Args:
        - S_t (torch.Tensor): The tensor of current states.
        - A_t (torch.Tensor): The tensor of actions.
        - S_t1 (torch.Tensor): The tensor of next states.
        - d_s (int): The dimension of the factorized state.
        - lambda1 (float): L1 regularization parameter.
        - lambda2 (float): L2 regularization parameter.
        - epsilon (float): Threshold for the causal graph reduction.

        Returns:
        - np.ndarray: The reduced adjacency matrix.
        """
        # Create the feature matrix
        X = self.create_feature_matrix(S_t, A_t, S_t1)

        # Define the model
        model = NotearsMLP(dims=[2*d_s + 1, 10, 1], bias=True)
        
        # Train the model with the NOTEARS algorithm
        W_est = notears_nonlinear(model, X.numpy(), lambda1=lambda1, lambda2=lambda2)
        
        # Perform causal graph reduction
        W_reduced = self.Reduce(torch.from_numpy(W_est), epsilon)
        
        return W_reduced.numpy()
