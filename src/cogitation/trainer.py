...
from cogitation.mpc.mpc import MPC_Chemistry
from cogitation.gnn_modules import RGCN, MLP, GRU_SCM
from cogitation.cogitation_utils import CUDA, kaiming_init
...
class WorldModel(object):
    def __init__(self, args):
        ...
        self.cogitation_model = args['cogitation_model']
        self.use_discover = args['use_discover']
        self.use_gt = args['use_gt']

        assert self.cogitation_model in ['causal', 'counterfact', 'mlp', 'offline', 'gnn']
        ...
        self.use_counterfact = False
        self.use_mlp = False
        if self.cogitation_model == 'mlp':
            self.model_name = 'mlp'
            self.use_mlp = True
        elif self.cogitation_model == 'causal':
            self.model_name = 'gru'
        elif self.cogitation_model == 'counterfact':
            self.model_name = 'gru'
            self.use_counterfact = True
        elif self.cogitation_model == 'gnn':
            self.model_name = 'gnn'
            self.use_counterfact = True
        ...
        print('COGITATION model:', self.cogitation_model)
        print('State abstraction:', self.use_state_abstraction) 
        print('Counterfact:', self.use_counterfact)
        ...
        if self.cogitation_model == 'causal':
    ...
    def build_node_and_edge_chemistry(self, data):
        ...
        if self.use_counterfact:
            # counterfact graph
            counterfact = np.ones((self.node_num, self.node_num))
            action_row = np.zeros((1, self.node_num))
            action_row[0] = 1
            counterfact[0, :] = action_row
            adj = counterfact
